...
## Artifactory (or changing pip repository location)
Setting the environment variable PYPI_URL (either directly or in env.list) will override the default pypi url when installing requirements from requirements.txt

Handling authentication:
    PYPI_USER: environment variable to use for pypi repo user
    There are two ways to send password for the pypi repo:
        1) Directly via PYPI_PASSWORD environment variable
        2) Setting PYPI_PASS_SECRET to aws secretsmanager path for password
    Setting these values will replace PYPI_USER:PYPI_PASSWORD in PYPI_URL and use that for pip install.
    Example:
        PYPI_USER=testuser
        PYPI_PASS_SECRET=testsecret/read
        PYPI_URL=https://PYPI_USER:PYPI_PASSWORD@artifactory-prod.domain.com/artifactory/api/pypi/pypi-virtual/simple
